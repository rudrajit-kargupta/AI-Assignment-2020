...
#    print(corate_variation_acc)    
    ...
    fin_str += "\n\n" + str(header)
    label['text'] = fin_str
    # plot fitness vs n_gen
    #plt.plot(f_hist,n_gen)
    ...
    pickle.dump(clf, open(filename, 'wb'))
    #nn_model =  tf.keras.Sequential([tf.keras.layers.Flatten(),tf.keras.layers.Dense(32, activation='relu'),
    #                                      tf.keras.layers.Dense(7)])
    #nn_model.compile(optimizer='adam',loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
    #              metrics=['accuracy'])
    #
    #nn_model.fit(X_train.values, y_train, epochs=50)
    #test_loss, test_acc = nn_model.evaluate(X_test,  y_test, verbose=2)
...
frame.place(relx=0.5, rely=0.1, relwidth=0.90, relheight=0.085, anchor='n')
...
frame2.place(relx=0.5, rely=0.3, relwidth=0.90, relheight=0.085, anchor='n') 
...
lower_frame.place(relx=0.5, rely=0.45, relwidth=0.9, relheight=0.5, anchor='n')
